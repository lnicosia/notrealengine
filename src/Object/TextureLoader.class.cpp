...
namespace notrealengine
{
	...
	std::shared_ptr<Texture>	TextureLoader::loadTexture(std::string path, unsigned char *data, unsigned int size, std::string type)
	{
		//	Manual check
		//	Verifiy if the map does not already
		//	contain an equivalent path
		...
		std::shared_ptr<Texture>	ptr(new Texture(path, data, size, type));
		textures.emplace(std::make_pair(path, ptr));
		//std::cout << "Texture '" << path << "' shared_ptr created" << std::endl;
		return ptr;
	}
